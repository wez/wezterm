...
impl WaylandWindowInner {
    ...
    fn dispatch_pending_event(&mut self) {
        ...
        let needs_decorations = self.config.window_decorations != WindowDecorations::NONE
            && pending
                .window_configure
                .as_ref()
                .is_some_and(|c| c.decoration_mode == DecorationMode::Client);

        if pending.configure.is_none() {
            if pending.dpi.is_some() || (self.decorations.is_none() && needs_decorations) {
                // Synthesize a pending configure event for the dpi change
                // or creation of decorations
        ...
        if let Some(window_config) = pending.window_configure {
            if self.window.is_some() && needs_decorations {
            ...
            if self.window.is_some() {
                ...
                match self.decorations.as_mut() {
                    Some(frame) if !frame.is_hidden() => {
                        ...
                        log::trace!("Resizing frame {},{} {:?}", x, y, outer_size);
