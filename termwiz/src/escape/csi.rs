...
impl DeviceAttributeFlags {
    ...
    fn from_params(params: &[i64]) -> Self {
        ...
        for p in params {
            match num_traits::FromPrimitive::from_i64(*p) {
...
impl<T: ParamEnum + PartialEq + num_traits::ToPrimitive> EncodeCSIParam for T {
    fn write_csi(&self, f: &mut Formatter, control: &str) -> Result<(), FmtError> {
...
/// primitive integer.
trait ParamEnum: num_traits::FromPrimitive {
    fn default() -> Self;
}
...
impl<T: ParamEnum> ParseParams for T {
    fn parse_params(params: &[i64]) -> Result<Self, ()> {
        if params.is_empty() {
            ...
            num_traits::FromPrimitive::from_i64(params[0]).ok_or(())
        } else {
            Err(())
        }
    }
    ...
    fn fmt(&self, f: &mut Formatter) -> Result<(), FmtError> {
        ...
        macro_rules! ansi_color {
            ($idx:expr, $eightbit:ident, $( ($Ansi:ident, $code:ident) ),*) => {
                if let Some(ansi) = num_traits::FromPrimitive::from_u8($idx) {
                    match ansi {
...
impl<'a> CSIParser<'a> {
    ...
    fn cursor_style(&mut self, params: &'a [i64]) -> Result<CSI, ()> {
        if params.len() != 1 {
            ...
            match num_traits::FromPrimitive::from_i64(params[0]) {
                None => Err(()),
                Some(style) => {
